...
const runScriptInInstance = async (instanceId, id, inputText, inputFilePath) => {
  ...
  let key = inputFilePath;
  if (inputFilePath.startsWith(`${bucketName}/`)) {
    key = inputFilePath.substring(bucketName.length + 1);
  }
  
  console.log('Adjusted key for S3 GetObjectCommand:', key);

  const s3GetObjectCommand = new GetObjectCommand({ Bucket: process.env.BUCKET_NAME, Key: inputFilePath });
