...
export const handler = async (event) => {
  ...
  for (const record of event.Records) {
    ...
    let skey = inputFilePath;
    if (inputFilePath.startsWith(`${bucketName}/`)) {
      skey = inputFilePath.substring(bucketName.length + 1);
    }
    
      const fileExists = await checkIfFileExists(skey);
      if (!fileExists) {
...
const checkIfFileExists = async (key) => {
  try {
    await s3Client.send(new HeadObjectCommand({ Bucket: process.env.BUCKET_NAME, Key: key }));
    return true;
  } catch (error) {
