...
const runScriptInInstance = async (instanceId, id, inputText, inputFilePath) => {
  ...
  const outputFilePath = inputKey.replace(originalExtension, '.output');
  ...
  const commands = [
    ...
    `bash /home/ec2-user/inputfile${originalExtension}`, // Execute the script (assuming it's a bash script)
    `echo "${outputFileContent}" > /home/ec2-user/outputfile.output` // Create the output file with appended length
  ];
