...
const checkInstanceState = async (instanceId) => {
    while (true) {
        const describeInstanceParams = { InstanceIds: [instanceId] };
        const describeInstancesCommand = new DescribeInstancesCommand(describeInstanceParams);
        const instanceData = await ec2Client.send(describeInstancesCommand);
        ...
        if (state === 'running') {
            ...
            console.log(`EC2 instance ${instanceId} is in state ${state}, waiting...`);
    ...
    for (const record of event.Records) {
        ...
        if (record.eventName === 'INSERT') {
            try {
                ...
                await checkInstanceState(instanceId); // Poll until the instance is running
