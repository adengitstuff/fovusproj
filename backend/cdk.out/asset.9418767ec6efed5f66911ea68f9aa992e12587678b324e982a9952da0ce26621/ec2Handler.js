...
/** Delay function to add a stagger to let ec2 instance develop from pending to running. */
    ...
    for (const record of event.Records) {
        ...
        if (record.eventName === 'INSERT') {
            try {
                ...
                /** Consistently ran into a problem of the request being rejected
                 * because the ec2 instance would take variable amounts of time to start up. Implemented a delay
                 * function, but still ran into the issue. I think one simple solution for now is to 
                 * simply implement a hard wait time, but this is a case where, in a production environment,
                 * I would ask a mentor or experienced veteran.
                 */
                console.log(`EC2 instance ${instanceId} created, waiting before deploying commands`);

                await delay(20000); // Wait for 10 seconds to let server spin up
