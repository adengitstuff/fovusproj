...
const runScriptInInstance = async (instanceId, id, inputText, inputFilePath) => {
  ...
  // Extract the file extension and create the output file path with .output extension. I use .output
  //  just to clearly differentiate between the input and output file
  const originalExtension = inputKey.substring(inputKey.lastIndexOf('.'));
  // Full output file path, to modify dynamoDB FileTable
  const outputFilePath = `${bucketName}/${id}/${inputKey.replace(originalExtension, '.output')}`;
  // Make a separate path for S3 to avoid the redundant folder issue
  const s3FilePath = `${id}/${inputKey.replace(originalExtension, '.output')}`;
