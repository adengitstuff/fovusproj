...
const runScriptInInstance = async (instanceId, id, inputText, inputFilePath) => {
  ...
  const commands = [
    ...
    `echo "${inputText.length}" >> /home/ec2-user/inputfile.input`, // Append the input text length
    `echo "${inputFileContent} : ${inputText.length}" > /home/ec2-user/outputfile${inputFilePath.substring(inputFilePath.lastIndexOf('.'))}`, // Create the output file with original extension
  ];
  ...
  // Step 3: Upload the output file to S3
  const outputFilePath = inputFilePath.replace(`${bucketName}/`, '').replace('.input', '.output');

  await s3Client.send(new PutObjectCommand({
    Bucket: process.env.BUCKET_NAME,
    Key: outputFilePath,
    Body: inputFileContent + ' : ' + inputText.length, // Combine input file content and text length for output
  }));

  // Step 4: Update DynamoDB with the output file path
